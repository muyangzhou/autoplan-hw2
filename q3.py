...
play.add_precondition(toy_free())

play.add_effect(toy_free(), False)
play.add_effect(has_played(k_p), True)

# -------- FREE TOY --------
free_toy = InstantaneousAction("free_toy", k=Kitten)
k_f = free_toy.parameter("k")
free_toy.add_precondition(current_room(k_f, playroom_const))
free_toy.add_precondition(has_played(k_f))
free_toy.add_effect(toy_free(), True)
...
def build_base_problem():
    ...
    problem.add_fluent(toy_free)

    # Add actions
    problem.add_action(move)
    problem.add_action(drink)
    problem.add_action(play)
    problem.add_action(free_toy)
    ...
    problem.set_initial_value(toy_free(), True)
...
def write_family():
    ...
    writer.write_domain("q3/domain.pddl")

    # Generate 5 increasing instances
    for i in range(1, 6):
        problem = generate_instance(i)
        writer = PDDLWriter(problem)
        writer.write_problem(f"q3/problem{i}.pddl")

    print("q3 family generated successfully.")
